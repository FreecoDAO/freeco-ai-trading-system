...
rm -f /tmp/ai-signal-generator.log /tmp/hummingbot-dashboard.log
touch /tmp/ai-signal-generator.log /tmp/hummingbot-dashboard.log
chmod 666 /tmp/ai-signal-generator.log /tmp/hummingbot-dashboard.log
...
if pgrep -f "ai-signal-generator" > /dev/null 2>&1; then
  ...
  echo "✗ AI Signal Generator failed - checking logs..."
  head -5 /tmp/ai-signal-generator.log
fi
echo ""
...
if pgrep -f "python3.*server.py" > /dev/null 2>&1; then
  ...
  echo "✗ Dashboard failed - checking logs..."
  head -5 /tmp/hummingbot-dashboard.log
fi
echo ""
...
if [ "$HTTP" = "200" ]; then
  ...
  echo "⚠ HTTP not responding yet (may be starting)"
else
  echo "✗ HTTP $HTTP"
fi
echo ""
...
if [[ "$MQTT" == "✓ RUNNING" ]] && [[ "$AI" == "✓ RUNNING" ]] && [[ "$DASH" == "✓ RUNNING" ]]; then
  ...
  echo "Next steps:"
  echo "  1. git add ."
  echo "  2. git commit -m 'feat: HTTP dashboard, complete setup'"
  echo "  3. git push origin main"
else
  echo "❌ Some services failed"
  echo ""
  echo "Logs:"
  echo "  cat /tmp/ai-signal-generator.log"
  echo "  cat /tmp/hummingbot-dashboard.log"
fi
